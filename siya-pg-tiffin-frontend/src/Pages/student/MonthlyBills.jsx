                   ...
                   <tr key={bill._id} className="hover:bg-orange-50/30 transition-colors group">
                     ...
                     <td className="px-6 py-6 font-semibold uppercase text-xs tracking-wider text-gray-500">
                        <span className={`inline-block px-2 py-1 rounded-md ${
                          bill.type === 'room' ? 'bg-blue-50 text-blue-600' : 
                          bill.type === 'monthly' ? 'bg-indigo-50 text-indigo-600' :
                          'bg-orange-50 text-orange-600'
                        }`}>
                           {bill.type === 'monthly' ? 'Combined (Room+Food)' : bill.type}
                        </span>
                        {bill.details && (
                           <div className="text-[10px] text-gray-400 mt-1 capitalize font-normal max-w-[200px] truncate" title={bill.details}>
                             {bill.details}
                           </div>
                        )}
                     </td>
           ...
           <div className="md:hidden divide-y divide-gray-100">
             {bills.map((bill) => (
                <div key={bill._id} className="p-6 relative">
                  ...
                  <div className="text-xl font-black text-gray-800 mb-1 capitalize flex items-center gap-2">
                     ₹{bill.amount} <span className="text-gray-300 font-light">&middot;</span> <span className={`text-sm ${bill.type === 'room' ? 'text-blue-500' : bill.type === 'monthly' ? 'text-indigo-500' : 'text-orange-500'}`}>{bill.type === 'monthly' ? 'Combined' : bill.type}</span>
                  </div>
                  {bill.details && <div className="text-xs text-gray-500 mb-4">{bill.details}</div>}
