...
import { FiUser, FiMail, FiPhone, FiMapPin, FiLock, FiEye, FiEyeOff, FiEdit3, FiCheckCircle, FiAlertCircle, FiShield, FiMenu } from "react-icons/fi";
...
const Profile = () => {
  ...
  const handleVerifyCurrentPassword = async () => {
    ...
    try {
      ...
      await API.post("/customer/profile/verify-password", { currentPassword: formData.currentPassword });
  ...
  const profileFields = [
    ...
    { icon: <FiMail className="text-orange-500" />, label: "Email", value: user?.email },
    { icon: <FiPhone className="text-orange-500" />, label: "Phone", value: user?.phone || "Not provided" },
    { icon: <FiMapPin className="text-orange-500" />, label: "Address", value: user?.address || "Not provided" },
    { icon: <FiMenu className="text-orange-500" />, label: "Delivery Preference", value: user?.deliveryPreference || "Both", capitalize: true },
          ...
          {!editMode ? (
                ...
                {profileFields.map((field, idx) => (
                  <div key={idx} className="flex items-center gap-4 py-4">
                    ...
                    <div>
                      ...
                      <p className={`text-gray-800 font-medium ${field.capitalize ? "capitalize" : ""}`}>{field.value}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 mt-4">
              ...
              <div>
                ...
                <textarea name="address" value={formData.address} onChange={handleInputChange} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-orange-500 outline-none font-medium resize-none" rows="2" placeholder="Optional" />
              </div>
