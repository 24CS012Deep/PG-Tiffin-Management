      ...
      try {
        const mealRows = record.dailyRecords.map(r =>
          `<tr>
            ...
            <td style="padding:6px 12px;border:1px solid #e5e7eb;text-align:center">${r.breakfast ? '✅' : '—'}</td>
            <td style="padding:6px 12px;border:1px solid #e5e7eb;text-align:center">${r.lunch ? '✅' : '—'}</td>
            <td style="padding:6px 12px;border:1px solid #e5e7eb;text-align:center">${r.dinner ? '✅' : '—'}</td>
        ...
        console.log("⚠️ Bill email failed:", emailErr.message);
      }
    }
